...
function ViewButton({ onClick, disabled = false }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
  ...
  return (
    <div className={`px-4 text-center ${mobile ? 'py-8' : 'py-12'}`}>
      ...
      <p className="mt-1 text-xs text-gray-500">
        {hasFilters ? 'Try adjusting your filters or clearing the search.' : 'No projects match the selected status.'}
      </p>
      {hasFilters && (
          ...
          <div className="bg-gray-50 p-4 md:p-6 border-b border-gray-100">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-lg md:text-xl font-semibold text-gray-900">Compliance Review</h2>
                <p className="text-xs md:text-sm text-gray-600 mt-0.5 md:mt-1">Manage and track project compliance</p>
              </div>
            </div>
          </div>
                  ...
                  {data.map((project) => {
                    ...
                    <   td className="px-6 py-4 text-sm justify-center text-gray-900 text-center">{project.project_id}</td>
                        ...
                        <td className="px-6 py-4 text-center">
                          <a href={route('compliance.show', project.project_id)}>
                            <ViewButton />
                          </a>
                        </td>
                      </tr>
              ...
              data.map((project) => {
                ...
                return (
                  <div key={project.project_id} className="p-3 space-y-2">
                    ...
                    <a href={route('compliance.show', project.project_id)}>
                      <button className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors mt-1">
