...
export default function UserManagement({ users, offices, filters, stats }) {
    ...
    const confirmAdminPassword = (e) => {
        ...
        router.put(
            ...
            { ...form, admin_password: adminPassword },
            {
                onSuccess: () => {
                    setEditingUser(null);
    ...
    const restoreUser = () => {
        router.put(
            `/admin/users/${restoreUserId}/restore`,
            {},
            {
                onSuccess: () => {
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {users.data.map((user) => {
                                    ...
                                    const avatarCls = getAvatarColor(
                                        user.first_name,
                                    );
                                    return (
                                        <tr
                                            key={user.user_id}
                                            className={`hover:bg-gray-50/70 transition-colors ${user.deleted_at ? "opacity-50" : ""}`}
                                        >
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div
                                                        className={`w-9 h-9 rounded-full ${avatarCls} flex items-center justify-center text-xs font-semibold flex-shrink-0`}
                                                    >
                                                        {getInitials(
                                                            user.first_name,
                                                            user.last_name,
                                            ...
                                            <td className="px-5 py-4">
                                                <span
                                                    className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium ring-1 ${badge.cls}`}
                                                >
                                                    {badge.label}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-5 py-4">
                                                <span className="inline-flex items-center gap-1.5 text-xs font-medium">
                                                    <span
                                                        className={`w-1.5 h-1.5 rounded-full ${user.is_online ? "bg-emerald-500" : "bg-gray-300"}`}
                                                    />
                                                    <span
                                                        className={
                                                            user.is_online
                    ...
                    <div className="md:hidden divide-y divide-gray-100">
                        {users.data.map((user) => {
                            ...
                            return (
                                <div
                                    key={user.user_id}
                                    className={`p-4 ${user.deleted_at ? "opacity-50" : ""}`}
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div
                                                className={`w-9 h-9 rounded-full ${avatarCls} flex items-center justify-center text-xs font-semibold flex-shrink-0 relative`}
                                            >
                                                {getInitials(
                                                    user.first_name,
                                                    user.last_name,
                                                ...
                                                <span
                                                    className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${user.is_online ? "bg-emerald-500" : "bg-gray-300"}`}
                                                />
                                            </div>
                                    ...
                                    <div className="flex items-center gap-2 mt-3 flex-wrap">
                                        <span
                                            className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ring-1 ${badge.cls}`}
                                        >
                                            {badge.label}
                                        </span>
                ...
                >
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                Enter your admin password to save changes
                            </p>
                        </div>
                    </div>
                    <form onSubmit={confirmAdminPassword} className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            ...
                            <button
                                type="submit"
                                disabled={isConfirming}
                                className={`${primaryBtn} min-w-[90px]`}
                            >
                                {isConfirming ? (
            ...
            {deleteUserId && (
                ...
                >
                    ...
                    <p className="text-xs text-gray-500 mb-5 bg-red-50 border border-red-100 rounded-lg p-3">
                        The user will be deactivated and their sessions
                        terminated. You can restore them later.
                    </p>
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            router.post(
                                `/admin/users/${deleteUserId}/delete`,
                    ...
                    >
                        <PasswordInput
                            ...
                            onChange={setConfirmDeletePassword}
                ...
                >
                    ...
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            router.post(
                                `/admin/users/${forceLogoutUserId}/logout`,
                    ...
                    >
                        <PasswordInput
                            ...
                            onChange={setConfirmLogoutPassword}
                            show={showLogoutPassword}
                            toggle={() =>
                                setShowLogoutPassword(!showLogoutPassword)
                            }
                            placeholder="Your admin password"
                            required
                        />
            ...
            {restoreUserId &&
                (() => {
                    const user = users.data.find(
                        (u) => u.user_id === restoreUserId,
                    );
                    return (
                        <Modal onClose={() => setRestoreUserId(null)}>
                            <div className="flex items-center gap-3 mb-2">
                            ...
                            <p className="text-xs text-gray-500 mb-5 bg-emerald-50 border border-emerald-100 rounded-lg p-3">
                                This will reactivate the account and allow the
...
// ── Shared styles ──
const selectCls =
    "w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent bg-gray-50 focus:bg-white outline-none transition-colors";
const cancelBtn =
    "px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors";
const primaryBtn =
    "px-4 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 transition-colors";
